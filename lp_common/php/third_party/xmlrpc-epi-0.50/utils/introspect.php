            ...
            if (is_array($val)) {
               ...
               foreach($val as $key2 => $desc) {
                  if (gettype($desc) === "string") {
                     ...
                     $buf .= "<tr>" . field(ucfirst($key2), $desc, 3, false) . "</tr>";
                  }
               }
            }
            else {
               $buf .= "<tr>" . field(ucfirst($key), $val, 3, false) . "</tr>";
            }
         }
      }
...
function field($title, $text, $columns=false, $width=false, $boldtitle = true) {
   if ($columns) {
   ...
   $colon = $title && $text ? ":" : "";
   $title = $boldtitle ? "<b>$title</b>" : $title;
   return "<td$colspan$width>$title$colon $text</td>";
}
